...
void static writecommand(uint8_t addr, uint8_t cmd) {
  while((SSI2_SR_R&SSI_SR_TNF)==0){};    // wait until SSI2 not busy/transmit FIFO empty
  SSI2_DR_R = 0x7D;                        // Change Address Command
  
		
	while((SSI2_SR_R&SSI_SR_TNF)==0){};    
  SSI2_DR_R = addr;                        // Address Location
 
		
	while((SSI2_SR_R&SSI_SR_TNF)==0){};   
  SSI2_DR_R = 0x7E;                        // Write Command 
		 
	while((SSI2_SR_R&SSI_SR_TNF)==0){};   
  SSI2_DR_R = cmd;                        // Change Address Command
...
int static readInput(void){
  ...
  SSI2_DR_R = 0x03;                        // Address Location
 while((SSI2_SR_R&SSI_SR_BSY)==SSI_SR_BSY){}; 

	 while((SSI2_SR_R&SSI_SR_BSY)==SSI_SR_BSY){};  
		SSI2_DR_R = 0x7F;                        // Read Address Command
		while((SSI2_SR_R&SSI_SR_BSY)==SSI_SR_BSY){};            // Read Address Command
...
void input_Init(){
 ...
 /* // toggle RST low to reset; CS low so it'll listen to us
  ...
  // initialize SSI0
  GPIO_PORTB_AFSEL_R |= 0x70;           // enable alt funct on PB 4,5,6, 7
  GPIO_PORTB_DEN_R |= 0xF0;             // enable digital I/O on PA2,3,5
                                        // configure PA2,3,5 as SSI
	GPIO_PORTB_DIR_R |=0x80;
  GPIO_PORTB_PCTL_R = (GPIO_PORTB_PCTL_R&0xF000FFFF)+0x02220000;
...
//  SSI0_CPSR_R = (SSI0_CPSR_R&~SSI_CPSR_CPSDVSR_M)+16;
  ...
  SSI2_CR1_R |= SSI_CR1_SSE;            // enable SSI
	...
	writecommand(0x71,0x00); //Set LED to negative logic
	writecommand(0x72, 0xFF); //link LED to sensor input
